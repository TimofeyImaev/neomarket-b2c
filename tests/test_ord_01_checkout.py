...
POST /api/v1/orders, тело {idempotency_key, items, delivery_address?}.
...
def _order_body(sku_id, qty=2, key=None):
    return {"idempotency_key": key or str(uuid.uuid4()),
...
def test_checkout_creates_paid_order_with_fixed_prices(app_client, fake_b2b):
    ...
    r = app_client.post("/api/v1/orders", headers=auth_headers(token), json=_order_body(sku, qty=2))
...
def test_partial_reserve_failure_returns_409(app_client, fake_b2b):
    ...
    r = app_client.post("/api/v1/orders", headers=auth_headers(token), json=_order_body(sku))
...
def test_idempotency_returns_existing_order(app_client, fake_b2b):
    ...
    sku = fake_b2b.add_sku(str(uuid.uuid4()), str(uuid.uuid4()), price=1000, available=10)
    key = str(uuid.uuid4())
    body = _order_body(sku, qty=1, key=key)
    r1 = app_client.post("/api/v1/orders", headers=auth_headers(token), json=body)
    r2 = app_client.post("/api/v1/orders", headers=auth_headers(token), json=body)
...
def test_checkout_requires_auth_401(app_client, fake_b2b):
    r = app_client.post("/api/v1/orders", json=_order_body(str(uuid.uuid4())))
...
def test_idempotency_same_key_different_body_returns_409(app_client, fake_b2b):
    ...
    sku = fake_b2b.add_sku(str(uuid.uuid4()), str(uuid.uuid4()), price=1000, available=10)
    key = str(uuid.uuid4())
    app_client.post("/api/v1/orders", headers=auth_headers(token), json=_order_body(sku, qty=1, key=key))
    r = app_client.post("/api/v1/orders", headers=auth_headers(token), json=_order_body(sku, qty=2, key=key))
    assert r.status_code == 409
